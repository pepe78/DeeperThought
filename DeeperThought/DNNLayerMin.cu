...
__global__ void min_forward(float *outp, const float *inp, int numPics, int x1, int x2, int d1, int d2, int batchSize)
{
...
__global__ void min_backward(float *dinp, const float *doutp, const float *outp, const float *inp, int numPics, int x1, int x2, int d1, int d2, int batchSize)
{
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
DNNLayerMin::DNNLayerMin(int _numPics, int _x1, int _x2, int _d1, int _d2, int _batchSize)
...
DNNLayerMin::~DNNLayerMin()
{

}

void DNNLayerMin::Forward(CPUGPUMemory* input)
{
	int threadsPerBlock = 256;
	int numBlocks = ((input->GetSize() / inputWidth) + threadsPerBlock - 1) / threadsPerBlock;
	min_forward<<<numBlocks, threadsPerBlock>>>(
		(float*)output->GetGPUMemory(), (float*)input->GetGPUMemory(), numPics, x1, x2, d1, d2, (input->GetSize() / inputWidth));
}

void DNNLayerMin::Backward(CPUGPUMemory* input, CPUGPUMemory* deltaOutput)
{
	if (deltaInput != NULL)
	{
		int threadsPerBlock = 256;
		int numBlocks = ((input->GetSize() / inputWidth) + threadsPerBlock - 1) / threadsPerBlock;
		min_backward << <numBlocks, threadsPerBlock >> > ((float*)deltaInput->GetGPUMemory(), (float*)deltaOutput->GetGPUMemory(),
			(float*)output->GetGPUMemory(), (float*)input->GetGPUMemory(), numPics, x1, x2, d1, d2, (input->GetSize() / inputWidth));
	}
}
